...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6,
        ...
        self.middle_model = nn.Sequential(*model)
        ...
        for i in range(n_downsampling):  # add upsampling layers
            ...
            model += [nn.ConvTranspose2d(ngf * mult, int(ngf * mult / 2),
                      ...
                      nn.ReLU(True)]
        self.up_model = nn.Sequential(*model)

        model = [nn.ReflectionPad2d(3)]
        model += [nn.Conv2d(ngf, output_nc, kernel_size=7, padding=0)]
        model += [nn.Tanh()]
        self.out_model = nn.Sequential(*model)

        self.model = nn.Sequential(*[self.down_model, self.middle_model, self.up_model, self.out_model])

    def forward(self, input, lower=None):
        """Standard forward"""
        if lower is None:
        ...
        input = self.up_model(input)
        return self.out_model(input)
...
class EncoderDecoder(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False):
        ...
        model = []
        downconv = nn.Conv2d(input_nc, ngf, kernel_size=4,
                             stride=2, padding=1, bias=use_bias)
        downrelu = nn.LeakyReLU(0.2, True)
        downnorm = norm_layer(ngf)
        model += [downconv, downnorm]

        for i in range(num_downs):
            mult = 2 ** i
            if mult >= 8:
                downconv = nn.Conv2d(ngf * 8, ngf * 8, kernel_size=4,
                                     stride=2, padding=1, bias=use_bias)
                downnorm = norm_layer(ngf * 8)
            else:
                downconv = nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=4,
                                     stride=2, padding=1, bias=use_bias)
                downnorm = norm_layer(ngf * mult * 2)
            downrelu = nn.LeakyReLU(0.2, True)
        ...
        for i in range(num_downs):
            mult = 2 ** (num_downs - i)
            if mult > 8:
                upconv = nn.ConvTranspose2d(ngf * 8, ngf * 8,
                                            kernel_size=4, stride=2,
                                            padding=1, bias=use_bias)
                upnorm = norm_layer(ngf * 8)
            else:
                upconv = nn.ConvTranspose2d(ngf * mult, int(ngf * mult / 2),
                                            kernel_size=4, stride=2,
                                            padding=1, bias=use_bias)
                upnorm = norm_layer(int(ngf * mult / 2))
            uprelu = nn.ReLU(True)
            up = [uprelu, upconv, upnorm]
            if use_dropout:
        ...
        upconv = nn.ConvTranspose2d(ngf, output_nc,
                                    kernel_size=4, stride=2,
                                    padding=1, bias=use_bias)
        up = [uprelu, upconv, nn.Tanh()]
